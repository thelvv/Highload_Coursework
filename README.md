...
## 3. Логическая схема  
![Структура БД](https://user-images.githubusercontent.com/59176894/235369056-d87112f2-1e9f-4b05-9fd3-e78f4693fb7b.jpg)

## 4. Физическая схема  
### Размер таблиц  

#### Пользоатели
208 байт/запись * 50 миллионов пользователей ~ 10 Гб
#### Категории
116 байт/запись * 500 категорий ~ 57 Кб
#### Объявления (основа)
128 байт/запись * 140 миллионов объявлений ~ 17 Гб
#### Объявления (часто меняющиеся данные)
1110 байт/запись * 140 миллионов объявлений ~ 145 Гб
#### Чаты
32 байт/запись * 140 миллионов объявлений * 5 чатов/объявление ~ 21 Гб
#### Сообщения
80 байт/запись * 140 миллионов объявлений * 5 чатов/объявление * 20 сообщений  ~ 1043 Гб
#### Сессии
20 байт/запись * 50 миллионов пользователей * 2 сессии/пользователь ~ 2 Гб
#### Фотографии
1 Мб/фото * (50 миллионов пользователей + 140 миллионов объявлений * 10 фото/объявление) ~ 1382 Тб

### Шардирование
Имеет смысл разделение по двум факторам: занимаемый размер и частота обновления данных.  
Таким образом, таблицы __Пользоатели, Категории, Объявления (основа), Сессии__ будут как менее интенсивно изменяемые, и таблицы __Объявления (часто меняющиеся данные), Чаты, Сообщения__ будут как более интенсивно изменяемые.  
...
### Схема
![Схема БД](https://user-images.githubusercontent.com/59176894/235377329-67ee45be-ff32-4a70-abd9-79d1a4266564.jpg)
...
### Хранение данных
Для хранения __информации о сессиях__ выбрана NoSQL СУБД Redis, Redis, так как она является распространенным решением для данной задачи.  
Для хранения __основной информации__ выбран PostgreSQL, так как данная СУБД надежная и гибкая, а также имеет открытый исходный код  
Для хранения изображений выбрано местное S3-совместимое хранилище (например, VK cloud), так как датацентры и CDN расположены рядом с основной целевой аудиторией

## 6. Схема проекта
![Структура БД HL-Страница 3](https://user-images.githubusercontent.com/59176894/235381058-17a93aa7-dfcd-4dc2-8113-66b09bb93772.jpg)  
Балансировка между серверами NGINX обеспечивается DNS.  
Балансировка между микросервисами обеспечивается NGINX.

## 7. Список серверов
### NGINX
NGINX легко обеспечит обработку суммарные ~4000 RPS. Для отказоустойчивости и надежности используется два сервера.
...
### Микросервисы Auth, Users, Ads, Chat
Микросервисы, написанные на Golang, с запасом обеспечат необходимые 4000 RPS даже на не самых мощных северах  
Для отказоустойчивости и надежности серверы продублированы  
...
### Redis
Для хранения и получения данных сессий достаточно средней конфигурации сервера и большего объема RAM для хранения в нем информации о сессиях  
